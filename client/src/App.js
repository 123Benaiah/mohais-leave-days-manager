  ...
  if (result) {
    ...
    const exportSelectedCSV = async () => {
        ...
        try {
            ...
            const rows = selected.map((emp, idx) => [
                idx + 1,
                emp.employee_number || '-',
                emp.name,
                emp.total_days,
    ...
    }; // Close exportSelectedCSV function
    ...
    const exportFilteredCSV = async () => {
        ...
        try {
            ...
            // Use main filtered employees list for export
            const exportData = filteredEmployees.length > 0 ? filteredEmployees : employees;

            if (exportData.length === 0) {
                showToast('warning', 'No Data', 'No employees to export.');
                return;
            }
            ...
            link.setAttribute('href', url);
            link.setAttribute('download', `employees_filtered_${new Date().getTime()}.csv`);
            link.style.visibility = 'hidden';
            ...
            showToast('success', 'Downloaded', `${exportData.length} employee(s) exported as CSV.`);
        } catch (error) {
            showToast('error', 'Download Failed', 'Failed to download CSV file. Please try again.');
            console.error('CSV export error:', error);
        } finally {
    ...
    const exportFilteredPDF = async () => {
        ...
        try {
            ...
            // Use the utility to generate the PDF
            await generateEmployeeReportPDF(exportData);

            showToast('success', 'Downloaded', `${exportData.length} employee(s) exported as PDF.`);
        } catch (error) {
            console.error('PDF export error:', error);
            showToast('error', 'Download Failed', 'Failed to generate PDF. Please try again.');
        } finally {
